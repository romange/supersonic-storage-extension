...
FailureOrOwned<Sink> CreatePageSink(
  ...
  std::unique_ptr<std::vector<std::unique_ptr<ColumnWriter> > >
      serializers(new std::vector<std::unique_ptr<ColumnWriter> >());
  std::shared_ptr<PageBuilder> page_builder(
...
FailureOrOwned<Sink> CreatePageSink(
    ...
    std::unique_ptr<std::vector<std::unique_ptr<ColumnWriter> > >
        column_writers,
    std::shared_ptr<PageBuilder> page_builder) {
  std::unique_ptr<PageSink> page_sink(new PageSink(
      std::move(projector), std::move(page_stream_writer),
